-- SCHEMA ALIGNMENT: Current Project
-- Run this in the Supabase SQL Editor for your CURRENT connected project (the one causing errors).

-- 1. Create the 'waitlist' table
-- Enhanced with proper constraints, enums, and new columns (Instagram, etc.)
CREATE TABLE IF NOT EXISTS waitlist (
  ...
  linkedin_url TEXT,
  instagram_handle TEXT, -- New UI requirement
...
-- 2. Create Indexes (Safe if they exist)
...
-- 4. Set Policies (Drop first to ensure clean state)
...
-- 5. Data Migration (If 'waitlist_entries' exists)
-- This attempts to copy data from the old table to the new one, mapping columns safely.
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'waitlist_entries') THEN
    ...
    SELECT 
      ...
      zip_code,
      linkedin_url,
       -- Map legacy roles to new schema
      CASE 
    ...
    RAISE NOTICE 'Migrated data from waitlist_entries to waitlist';
  END IF;
END $$;
