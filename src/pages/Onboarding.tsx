    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            const userPayload = {
                ...
                schedule: data.schedule, // structured JSON
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Input
                            label="Zip Code"
                            value={data.zipCode}
                            onChange={v => updateData('zipCode', v.replace(/\D/g, '').slice(0, 5))}
                            required
                            placeholder="12345"
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-2 mb-4 p-3 bg-white border border-[#c8e6d9] rounded-lg">
                        <input
                            ...
                            onChange={e => updateData('scheduleFlexible', e.target.checked)}
                                ...
                                {['Morning', 'Afternoon'].map(time => (
                                    <React.Fragment key={time}>
                                        ...
                                        {['mon', 'tue', 'wed', 'thu', 'fri'].map(day => {
                                            const key = `${day}_${time.toLowerCase()}` // simple key logic
...
const SelectionCard = ({ label, desc, selected, onClick, multiple }: any) => (
    <div
        onClick={onClick}
        className={`p-4 rounded-xl border-2 transition-all cursor-pointer flex items-center gap-3
        ${selected
                ? 'border-[#1B4D3E] bg-[#d8f5e5] shadow-sm'
