...
export const ScheduleGrid = ({ value, onChange }: ScheduleGridProps) => {
  ...
  const toggle = (day: string, time: string) => {
    ...
    // Clean up empty arrays to keep JSON clean (optional, but good practice)
    if (newSchedule[dayLower].length === 0) {
      delete newSchedule[dayLower];
    }
  ...
  const handleQuickSelect = (q: any) => {
    ...
    for (const d of q.days) {
      ...
      if (isFullySelected) {
        // Remove
        const remaining = currentTimes.filter((t: string) => !q.times.includes(t));
        if (remaining.length > 0) {
          newSchedule[dayLower] = remaining;
        } else {
          delete newSchedule[dayLower];
        }
      } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-gray-200 rounded-xl p-2 md:p-4 bg-white overflow-x-auto">
        {/* Fixed min-width grid container for horizontal scrolling on mobile */}
