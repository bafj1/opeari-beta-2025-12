...
export default function ProfilePanel({ formData, setFormData, saving, onSave }: ProfilePanelProps) {
    ...
    return (
        <form onSubmit={(e) => { e.preventDefault(); onSave(); }} className="space-y-8 animate-fade-in max-w-4xl">
            ...
            {/* SECTION 5C: HOUSEHOLD DETAILS (Family Only) */}
            {viewer?.member?.role !== 'caregiver' && (
                <SettingsCard title="Household Details" description="Help caregivers know what to expect" icon={Heart}>
                    {/* Home Type */}
                    <div className="mb-4">
                        <label className="block text-xs font-bold text-[#1e6b4e] uppercase tracking-wide mb-2">Home Type</label>
                        <div className="flex flex-wrap gap-2">
                            {['House', 'Apartment', 'Condo', 'Townhouse'].map(type => {
                                const val = type.toLowerCase();
                                const isSelected = formData.home_type === val;
                                return (
                                    <button
                                        key={type}
                                        type="button"
                                        onClick={() => setFormData({ ...formData, home_type: isSelected ? null : val })}
                                        className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${isSelected
                                            ? 'bg-[#1e6b4e] text-white border-[#1e6b4e]'
                                            : 'bg-white text-[#546E5C] border-[#8bd7c7]/30 hover:border-[#8bd7c7]'
                                            }`}
                    ...
                    {/* Parking & Stairs toggles */}
                    <div className="space-y-3">
                        {[
                            { key: 'has_parking', label: 'Parking available', desc: 'Caregiver can park at or near your home' },
                            { key: 'has_stairs', label: 'Home has stairs', desc: 'Stairs between main living areas' },
                        ].map(item => {
                            ...
                            return (
                                <button
                                    key={item.key}
                                    type="button"
                                    onClick={() => setFormData({ ...formData, [item.key]: !isOn })}
                                    className="w-full flex items-center justify-between py-2 group"
                                >
                                    <div className="text-left">
                                        <p className="text-sm font-semibold text-[#1e6b4e]">{item.label}</p>
                                        <p className="text-xs text-[#546E5C]">{item.desc}</p>
                                    </div>
                                    <div className={`w-10 h-6 rounded-full transition-all duration-200 flex items-center px-0.5 ${isOn ? 'bg-[#1e6b4e]' : 'bg-gray-200'}`}>
                                        <div className={`w-5 h-5 rounded-full bg-white shadow transition-transform duration-200 ${isOn ? 'translate-x-4' : 'translate-x-0'}`} />
                                    </div>
                                </button>
                            );
                        })}
                    ...
                    {/* Budget Range */}
                    <div>
                        <label className="block text-xs font-bold text-[#1e6b4e] uppercase tracking-wide mb-2">Budget Range</label>
                        <p className="text-xs text-[#546E5C] mb-2">What you can pay per hour (optional, helps filter matches)</p>
                        <div className="flex items-center gap-3">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, budget_min: e.target.value ? parseInt(e.target.value) : null })}
                                    placeholder="20"
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData({ ...formData, budget_max: e.target.value ? parseInt(e.target.value) : null })}
                                    placeholder="40"
                                    className="w-24 pl-7 pr-3 py-2.5 border border-[#8bd7c7]/30 rounded-xl text-sm text-[#1e6b4e] focus:border-[#1e6b4e] focus:ring-1 focus:ring-[#1e6b4e]/20 focus:outline-none"
                                    min="0" max="200"
                                />
